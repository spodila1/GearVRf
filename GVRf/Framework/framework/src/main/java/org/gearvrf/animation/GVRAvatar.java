...
public class GVRAvatar extends GVRBehavior implements IEventReceiver
{
    ...
    /**
     * Interace for listenting to avatar events.
     */
    public interface IAvatarEvents extends IEvents
    {
        /**
         * Called when an avatar model has finished loading.
         * @param avatarRoot    root of loaded hierarchy, null if load failed
         * @param filePath      name of file
         * @param errors        load errors, null if successful
         */
        public void onAvatarLoaded(GVRSceneObject avatarRoot, String filePath, String errors);

        /**
         * Called when an animation has finished loading.
         * @param animation     {@link GVRAnimator} with animations(s)
         * @param filePath      name of file
         * @param errors        load errors, null if successful
         */
    ...
    /**
     * Load the model in the given resource.
     * <p>
     * The model is assumed to represent an avatar.
     * It is loaded without animations but it is rigged with a skeleton,
     * The previous avatar, if any, is removed.
     * <p>
     * After the model has been loaded, the IAvatarEvents.onAvatarLoaded function
     * is called on any listeners to notify the asset load is complete.
     * To listen for this event, add a listener to the {@link GVREventReceiver}
     * associated with this avatar (see {@link #getEventReceiver()}.
     * @param avatarResource resource containing avatar model
     */
    ...
    /**
     * Load the animation in the given resource.
     * <p>
     * The resource is assumed to contain an anination
     * for a rigged avatar. Some file formats may contain the avatar
     * skeleton, others may not.
     * <p>
     * After the animation has been loaded, the IAvatarEvents.onAnimationLoaded function
     * is called on any listeners to notify the asset load is complete.
     * To listen for this event, add a listener to the {@link GVREventReceiver}
     * associated with this avatar (see {@link #getEventReceiver()}.
     * @param animResource resource containing animation
     */
    ...
    /**
     * Center the current avatar model.
     */
    public void centerModel()
    {
        GVRSceneObject.BoundingVolume bv = mModelRoot.getBoundingVolume();
        float x = 0;
        float y = 0;
        float z = 0;
        float sf = 1 / bv.radius;
        mModelRoot.getTransform().setScale(sf, sf, sf);
        bv = mModelRoot.getBoundingVolume();
        mModelRoot.getTransform().setPosition(x - bv.center.x, y - bv.center.y, z - bv.center.z - 1.5f * bv.radius);
    }
